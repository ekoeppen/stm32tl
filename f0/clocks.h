...
struct HSI48_RC {
	static constexpr CLOCK_SOURCE_TYPE clock_source = HSI48_CLOCK_SOURCE;
	static constexpr uint32_t frequency = 48000000;
	static void init(void) {
...
struct PLL_T {
	...
	static void init(void) {
		RCC->CFGR = 0;
		if (CLOCK_SOURCE::clock_source == HSI_CLOCK_SOURCE) {
			RCC->CFGR |= (RCC_CFGR_PLLSRC_HSI_DIV2 | multiplier(CLOCK_SOURCE::frequency / 2, frequency));
		} else {
			RCC->CFGR |= (RCC_CFGR_PLLSRC_HSE_PREDIV | multiplier(CLOCK_SOURCE::frequency, frequency));
		}
		RCC->CR |= RCC_CR_PLLON;
...
struct SYSCLK_T {
	...
	static constexpr uint32_t pclk = frequency / APB_PRESCALER;
	static void init(void) {
		...
		static_assert(APB_PRESCALER == 1, "APB prescaler setting not implemented");
		switch (CLOCK_SOURCE::clock_source) {
		case HSI48_CLOCK_SOURCE:
